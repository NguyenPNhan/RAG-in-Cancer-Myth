# GPT-5.6 Terra non-RAG baseline

This directory provides the non-RAG baseline for the cancer-myth prompt
comparison. It classifies the same questions with the `gpt-5.6-terra` model,
the same prompt templates, concurrency settings, and strict Boolean JSON schema
as the RAG experiment, but it does **not** retrieve or send NCI PDQ passages.
...
- `answers_basic.csv`, `answers_oncology_expert.csv`, and
  `answers_patient_education.csv` store results and are initially empty apart
  from their headers.
...
uv run --with jupyterlab jupyter lab non_rag/gpt-5.6-terra
```

The runner fixes the experiment model to `gpt-5.6-terra`; other connection
settings still come from `LLMSettings` and its supported environment variables.

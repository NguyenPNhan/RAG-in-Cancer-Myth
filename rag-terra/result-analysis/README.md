# GPT-5.6 Terra result analysis

This folder contains the reproducible comparison of the three GPT-5.6 Terra
RAG runs and their matching Terra non-RAG baselines. The analyzer validates every input, joins
...
```powershell
uv run python rag-terra/result-analysis/analyze.py
```
...
rag-terra/rag_run_all/answers_{prompt}.csv
non_rag/gpt-5.6-terra/answers_{prompt}.csv
```

The command-line version accepts `--dataset`, `--rag-dir`, `--non-rag-dir`, or
`--output` overrides. Run `uv run python rag-terra/result-analysis/analyze.py --help` for
details.

## Outputs

Generated artifacts are written to `rag-terra/result-analysis/output/`:

# GPT-5.6 Terra batch RAG classification and evaluation
...
the local NCI PDQ corpus and calls OpenAI's `gpt-5.6-terra` model with strict
JSON-schema output.
...
`LLM_BASE_URL` and `LLM_API_KEY` can override the connection defaults, and
`LLM_API_KEY` takes precedence over `OPENAI_API_KEY`. The notebooks pin the
model to `gpt-5.6-terra`; `LLM_MODEL` is intentionally ignored.
